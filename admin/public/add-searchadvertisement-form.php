...
if (isset($_POST['btnAdd'])) {
	...
	$link = $db->escapeString(($_POST['link']));
        ...
        if (!empty($city) && !empty($link)) 
		{
				...
				// upload new image
				$upload = move_uploaded_file($_FILES['product_image']['tmp_name'], 'upload/searchadvertisement/' . $menu_image);

				// insert new data to menu table
				$upload_image = 'upload/searchadvertisement/' . $menu_image;
            
            $sql_query = "INSERT INTO searchadvertisement (city_id,link,image)VALUE('$city','$link','$upload_image')";
            $db->sql($sql_query);
            $result = $db->getResult();
            if (!empty($result)) {
                $result = 0;
            ...
            if ($result == 1) {
                ...
                $error['add_searchadvertisement'] = "<section class='content-header'>
                                                <span class='text text-success'>Advertisement Added Successfully</span> </section>";
            } else {
                $error['add_searchadvertisement'] = " <span class='text text-danger'>Failed</span>";
            }
            }
		}
?>

    <?php echo isset($error['add_searchadvertisement']) ? $error['add_searchadvertisement'] : ''; ?>
					...
					<div class="row gutters">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							...
							<div class="card">
								...
								<form name="add_searchadvertisement_form" method="post" enctype="multipart/form-data">
									<div class="card-body">
