...
if (isset($_POST['btnEdit'])) {
	...
	if ( !empty($city) && !empty($link) ) 
	{
			...
			if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
				...
				$target_path = 'upload/searchadvertisement/';
				...
				$upload_image = 'upload/searchadvertisement/' . $filename;

				$sql = "UPDATE searchadvertisement SET `image`='" . $upload_image . "' WHERE `id`=" . $ID;
				$db->sql($sql);
			}
			     $sql_query = "UPDATE searchadvertisement SET city_id='$city',link='$link' WHERE id=$ID";
				$db->sql($sql_query);
				$result = $db->getResult();
				if (!empty($result)) {
					$result = 0;
				...
				if ($result == 1) {
					...
					$error['update_searchadvertisement'] = "<section class='content-header'>
													<span class='text text-success'>Advertisement Updated Successfully</span> </section>";
				} else {
					$error['update_searchadvertisement'] = " <span class='text text-danger'>Failed</span>";
				}
				}
			}
...
$sql_query = "SELECT * FROM searchadvertisement WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "searchadvertisement.php";
	</script>
<?php } ?>

    <?php echo isset($error['update_searchadvertisement']) ? $error['update_searchadvertisement'] : ''; ?>
					...
					<div class="row gutters">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							...
							<div class="card">
								...
								<form name="edit_searchadvertisment_form" method="post" enctype="multipart/form-data">
									<div class="card-body">
